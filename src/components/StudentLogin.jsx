...
const StudentLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('/api/v1/students/login', payload, {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      const response = await axios.get('/api/v1/students/me', { withCredentials: true });
      if (response?.data?.success) {
        navigate('/student/dashboard')
      }
    }
          ...
          <div className="p-8">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-gray-500 dark:text-zinc-400 black:text-[#888] mt-1.5">
                Sign in as{" "}
                <span className="text-[#ba7a4e] font-semibold capitalize">Student</span>
              </p>
            </div>
